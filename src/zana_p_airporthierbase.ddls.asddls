...
define view entity ZANA_P_AirportHierBase
  as select from Zana_ap_h

  association [0..1] to ZANA_I_Airport         as _Airport on  _Airport.AirportID = $projection.AirportID

  association [0..1] to ZANA_I_City            as _City    on  _City.country = $projection.Country
                                                               and _City.city    = $projection.City

  association [0..1] to I_Country                  as _Country on  _Country.Country = $projection.Country

  association [0..1] to ZANA_I_AirportHierDir  as _Dir     on  _Dir.HierarchyID = $projection.HierarchyID

  association [0..1] to ZANA_I_AirportHierNode as _Node    on  _Node.HierarchyID = $projection.HierarchyID
                                                               and _Node.NodeName    = $projection.NodeName

  association [0..1] to ZANA_P_AirportHierBase as _Parent  on  _Parent.HierarchyID = $projection.HierarchyID
