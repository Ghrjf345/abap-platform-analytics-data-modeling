CLASS Zcl_data_gen_ana_extension DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      ...
      fill_airport_hier
        IMPORTING i_hieid TYPE Zana_airport_hieid,

      fill_airline_hier
        IMPORTING i_hieid TYPE Zana_carrier_hieid,
      ...
      get_continent
        ...
        RETURNING VALUE(r_continent) TYPE Zana_text,

      get_nodetext
        IMPORTING i_nodename        TYPE Zana_nodename
        RETURNING VALUE(r_nodetext) TYPE Zana_text.

ENDCLASS.



CLASS Zcl_data_gen_ana_extension IMPLEMENTATION.
  ...
  METHOD fill_city.
    ...
    DELETE FROM Zana_city.                          "#EC CI_NOWHERE
...
*               and the second coordinate is latitude with bounds -90 to 90.
    ...
    INSERT Zana_city FROM TABLE @i_ts_city.

    " fill geo-field with the help of DB-function
    UPDATE Zana_city SET geopoint = st_transform( st_new_point( longitude , latitude, 4326 ) , 3857 ) . "#EC CI_NOWHERE
  ...
  METHOD fill_airport_hier.
    ...
    p_r_log->print_delete( ).
    DELETE FROM Zana_ap_h  WHERE hierarchy_id = @i_hieid.
    DELETE FROM Zana_ap_hn WHERE hierarchy_id = @i_hieid.
    DELETE FROM Zana_ap_hd WHERE hierarchy_id = @i_hieid.
    ...
    p_r_log->print_insert( ).
    INSERT Zana_ap_h  FROM TABLE @l_t_hier.
    INSERT Zana_ap_hn FROM TABLE @l_t_hier_node.
    INSERT Zana_ap_hd FROM TABLE @l_t_hier_dir.
  ...
  METHOD generate_geo_hier_data.
    ...
    CONSTANTS: c_nodeid_world TYPE Zana_airport_nodeid VALUE '00001'.

    DATA: l_s_hier          TYPE pt_s_airport_hier,
          l_s_node          TYPE pt_s_airport_hier_node,
          l_t_airport       TYPE STANDARD TABLE OF Zairport,
          l_nodetext        TYPE Zana_text,
          l_node_id         TYPE Zana_airport_nodeid,
          l_sequence_number TYPE i.

    SELECT * FROM Zairport INTO TABLE @l_t_airport. "#EC CI_NOWHERE
...
*                 PARENT_NODE_ID initial == root!
  ...
  METHOD generate_abc_hier_data.
    CONSTANTS: c_nodeid_all TYPE Zana_airport_nodeid VALUE '00001'.

    DATA: l_s_hier          TYPE pt_s_airport_hier,
          l_s_node          TYPE pt_s_airport_hier_node,
          l_t_airport       TYPE STANDARD TABLE OF Zairport,
          l_nodename        TYPE Zana_nodename,
          l_nodename_p      TYPE Zana_nodename,
          l_nodetext        TYPE Zana_text,
          l_node_id         TYPE Zana_airport_nodeid,
          l_sequence_number TYPE i.

    " select all fields / performance is not issue here
    SELECT * FROM Zairport
      ORDER BY airport_id
