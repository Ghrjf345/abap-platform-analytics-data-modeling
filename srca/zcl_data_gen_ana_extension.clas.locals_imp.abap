...
CLASS lcl_airline_hier IMPLEMENTATION.
  ...
  METHOD init_by_cubedata.
    ...
    SELECT currency_code FROM Zcarrier GROUP BY currency_code INTO TABLE @p_ts_curr.

  ENDMETHOD.

  METHOD delete_data_from_db.
    DELETE FROM Zana_cr_s_h  WHERE hierarchy_id = @p_hieid.
    DELETE FROM Zana_cr_s_hd WHERE hierarchy_id = @p_hieid.

    DELETE FROM Zana_cr_t_h  WHERE hierarchy_id = @p_hieid.
    DELETE FROM Zana_cr_t_hd WHERE hierarchy_id = @p_hieid.
    DELETE FROM Zana_cr_t_hn WHERE hierarchy_id = @p_hieid.
  ENDMETHOD.

  METHOD save_data_on_db.
    INSERT Zana_cr_s_h  FROM TABLE @p_t_data_hier.
    INSERT Zana_cr_s_hd FROM TABLE @p_t_data_dir.

    INSERT Zana_cr_t_hn FROM TABLE @p_t_data_td_node.

    INSERT Zana_cr_t_h  FROM TABLE @p_t_data_td_hier.
    INSERT Zana_cr_t_hd FROM TABLE @p_t_data_td_dir.
  ENDMETHOD.
  ...
  METHOD get_parent_id.
    ...
    DATA: l_index      TYPE i,
          ...
          l_parentname TYPE Zana_nodename.
    FIELD-SYMBOLS:
  ...
  METHOD generate_carrier_hier_data.
    ...
    DATA: l_s_hier          TYPE pt_s_carrier_hier,
          ...
          l_t_carrier       TYPE STANDARD TABLE OF Zcarrier,
          l_nodetext        TYPE Zana_text,
          l_node_id         TYPE Zana_carrier_nodeid,
          l_sequence_number TYPE i,
          l_t_super_node    TYPE pt_t_carrier_hier_td_node,
          l_parent_id       TYPE Zana_carrier_nodeid,
          l_date_from       TYPE d,
          l_date_to         TYPE d,
...
* add leaves
    SELECT * FROM Zcarrier INTO TABLE @l_t_carrier. "#EC CI_NOWHERE
...
* directory
  ...
  METHOD generate_carrier_hier_td_data.
    DATA: l_datefrom TYPE Zana_date_from,
          l_dateto   TYPE Zana_date_to.
